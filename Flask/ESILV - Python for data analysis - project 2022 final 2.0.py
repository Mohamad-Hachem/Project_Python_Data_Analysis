...
# 
#  - The **lake of data** ("?" and "Nan" for exmaple)
...
# The data is cleaned. From now, we could manipulate it easily and visualize it through **plots** in order to find out some important predictors to fit the model.
...
# Let us see what are the correlation between them in order to pick up only the most valuable features to fit our model.
...
# we can see that there is few cases before the age of 40 and they peak around the age of 75
...
# We decided to allocate **2/3** of the data to training and **1/3** to the testing.
...
# To improve our model, we could split our testing set into 2 parts :
#  - **validation set**
#  - **testing set**
# 
...
# We can see that the score is about <ins>61-62,4%</ins>
...
predictors = [x for x in df.columns[0:len(df.columns)-1]]
print(predictors)
...
# In[79]:
import pickle
pickle.dump(gradient_boosting_model_new,open('GradientBoostingModel.pkl','wb'))
...
#  - The more there are features, the more the **Gradient** model is efficient.
# 
...
param_test = {'n_estimators':range(20,30,5)}
...
# We finally have a model based on **Gradient** wich is boosted by the model tuning thanks to a grid search. The best accucary that we obtained through this whole work is **62,5%**.
